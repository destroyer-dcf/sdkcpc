...
def rvm():
    ...
    retcode = subprocess.run([_rvm,"-i", DSK,"-b=cpc"+project_data['rvm']['model.cpc'],"-c=RUN\""+project_data["config"]["name.bas.file"]+"\n"])
    
    FNULL = open(os.devnull, 'w')
    try:
        retcode = subprocess.Popen([_rvm,"-i", DSK,"-b=cpc"+project_data['rvm']['model.cpc'],"-c=RUN\""+project_data["config"]["name.bas.file"]+"\n"], stdout=FNULL, stderr=subprocess.STDOUT)
        print()
    except:

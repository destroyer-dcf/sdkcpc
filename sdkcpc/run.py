...
def Download_RVM():
    if not os.path.exists(_rvm):
        ...
        open(path.dirname(path.abspath(__file__)) + "/resources/platform/" + sys.platform + "/rvm.zip", "wb").write(response.content)
        print(path.dirname(path.abspath(__file__)) + "/resources/platform/" + sys.platform + "/rvm.zip")
...
def rvm():
    ...
    console.print("[blue bold][Emulator][white] " + RVM)
